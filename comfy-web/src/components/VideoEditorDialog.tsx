...
export default function VideoEditorDialog({ isOpen, onClose, videoGallery, onSuccess }: VideoEditorDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative flex h-[90vh] w-full max-w-6xl flex-col rounded-[24px] border border-[#3f3e3a] bg-[#2a2a28] shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#3a3936] bg-[#2f2f2d] px-6 py-4 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-[#edeae2]">Video Editor</h2>
              <p className="text-xs text-[#9f988c]">Timeline-based NLE</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-xs text-[#6b6560]">
              Total: <span className="text-[#c9a87a] font-mono tabular-nums">{formatTime(duration)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex min-h-0 flex-1">
          {/* Source Clips Panel */}
          <div className="flex w-56 flex-col border-r border-[#3a3936] bg-[#262624] shrink-0">
            <div className="border-b border-[#3a3936] px-4 py-3">
              <p className="text-[10px] uppercase tracking-widest text-[#6b6560]">Source Clips</p>
              <p className="text-[10px] text-[#6b6560] mt-0.5">Click to add to timeline</p>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {remainingClips.length === 0 ? (
                <p className="text-xs text-[#6b6560] text-center py-8">All clips added</p>
              ) : (
                remainingClips.map((v) => (
                  <button
                    key={v.id}
                    onClick={() => handleAddClip(v)}
                    className="group w-full rounded-xl border border-[#3a3936] bg-[#2f2f2d] overflow-hidden text-left transition hover:border-[#5a554a] hover:bg-[#353533]"
                  >
                    ...
                    <div className="px-2.5 py-2">
                      <p className="truncate text-[11px] text-[#bcb6aa] leading-snug">{v.prompt}</p>
                      {v.resolution && (
                        <span className="mt-1 inline-block rounded bg-[#1a1a18] px-1 py-0.5 text-[9px] text-[#6b6560]">
                          {v.resolution}
                        </span>
                      )}
                    </div>
              ...
              {Object.keys(items).length === 0 ? (
                <div className="flex-1 flex items-center justify-center">
                  <div className="text-center">
                    <svg className="mx-auto mb-3 h-10 w-10 text-[#4a4944]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M11 4a.5.5 0 01.5-.5h1a.5.5 0 01.5.5v1a.5.5 0 01-.5.5h-1a.5.5 0 01-.5-.5V4zM12.5 8.5a.5.5 0 00-1 0v3.793l-1.146-1.147a.5.5 0 00-.708.708l2 2a.5.5 0 00.708 0l2-2a.5.5 0 00-.708-.708L12.5 12.293V8.5zM4 5.5A2.5 2.5 0 016.5 3h1A1.5 1.5 0 019 4.5V5h6v-.5A1.5 1.5 0 0116.5 3h1A2.5 2.5 0 0120 5.5v12a2.5 2.5 0 01-2.5 2.5h-13A2.5 2.5 0 012 17.5v-12z" />
                    </svg>
                    <p className="text-sm text-[#6b6560]">Click clips from the left panel to build your timeline</p>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex flex-col flex-1 min-h-0">
                    {/* Canvas preview */}
                    <div className="shrink-0 bg-[#1a1a18] px-4 py-3 border-b border-[#3a3936]">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0 pt-1">
                          <div className="flex items-center gap-3">
                            <span className="text-[11px] text-[#bcb6aa] font-medium">
                              {playing ? 'Playing' : 'Paused'}
                            </span>

                          </div>
                          <div className="mt-2 text-[11px] text-[#6b6560] leading-relaxed">
                            <span className="text-[#c9a87a] font-mono tabular-nums">{formatTime(currentTime)}</span>
                            {' / '}
        ...
        {/* Footer */}
        <div className="flex items-center justify-between border-t border-[#3a3936] bg-[#2f2f2d] px-6 py-4 shrink-0">
          <div className="flex items-center gap-4">
            <span className="text-[10px] text-[#6b6560]">Space: Play/Pause · S: Split · Del: Delete · ←→: Seek</span>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
              className="rounded-xl px-4 py-2 text-sm font-medium text-[#9f988c] transition hover:text-[#edeae2]"
            >
              Cancel
            </button>
            <button
